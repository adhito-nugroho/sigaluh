...
<div class="mb-6 flex items-center justify-between">
    <div>
        <h1 class="text-2xl font-bold text-slate-900"><?= $is_edit ? 'Edit KTH' : 'Tambah KTH Baru' ?></h1>
        <p class="text-sm text-slate-500 mt-1">Kelola master data Kelompok Tani Hutan.</p>
    </div>
    <a href="<?= BASE_URL ?>/index.php?page=kth" class="text-sm font-medium text-gray-600 hover:text-slate-900 flex items-center">
...
<div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
    <form action="<?= BASE_URL ?>/index.php?page=kth/process" method="POST" class="p-6 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Nama Kelompok Tani Hutan (KTH) <span class="text-red-500">*</span></label>
            ...
            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Nama Ketua</label>
                <input type="text" name="ketua" value="<?= $is_edit ? e($kth['ketua']) : '' ?>"
                    class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none text-sm transition-all focus:ring-2 focus:ring-brand-primary outline-none">
            </div>

            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Nomor SK Pengukuhan</label>
            ...
            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Kelas Kelompok</label>
                <select name="kelas_kelompok" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none text-sm transition-all focus:ring-2 focus:ring-brand-primary outline-none bg-white">
            ...
            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Jumlah Anggota</label>
                <input type="number" name="jumlah_anggota" min="0" value="<?= $is_edit ? e($kth['jumlah_anggota']) : '' ?>"
                    class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none text-sm transition-all focus:ring-2 focus:ring-brand-primary outline-none">
            </div>

            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Luas Lahan (Ha)</label>
                <input type="number" step="0.01" name="luas_lahan_ha" min="0" value="<?= $is_edit ? e($kth['luas_lahan_ha']) : '' ?>"
                    class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none text-sm transition-all focus:ring-2 focus:ring-brand-primary outline-none">
            </div>

            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Kontak KTH</label>
            ...
            <div class="md:col-span-2">
                ...
                <h3 class="text-sm font-semibold text-slate-900 mb-2">Wilayah Kedudukan KTH</h3>
                <?php if ($role === 'penyuluh'): ?>
                <div class="p-3 bg-indigo-50/80 border border-indigo-100 rounded-xl flex items-center text-xs font-semibold text-indigo-800 mb-2">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="keterangan" rows="3"
                    class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none text-sm transition-all focus:ring-2 focus:ring-brand-primary outline-none"><?= $is_edit ? e($kth['keterangan']) : '' ?></textarea>
            </div>

        </div>

        <div class="flex justify-end pt-4 border-t border-slate-200/80">
            <button type="submit" class="px-6 py-2 bg-brand-primary hover:bg-brand-secondary text-white rounded-lg font-medium transition-colors shadow-sm">
