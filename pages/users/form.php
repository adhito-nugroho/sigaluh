...
<div class="mb-6 flex items-center justify-between">
    <div>
        <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight"><?= $is_edit ? 'Edit Data Pengguna' : 'Tambah Pengguna Baru' ?></h1>
        <p class="text-xs font-medium text-slate-500 mt-1">Kelola informasi akun pengguna, role/peran, dan otentikasi login.</p>
    </div>
    <a href="<?= BASE_URL ?>/index.php?page=users" class="text-xs font-semibold text-slate-600 hover:text-slate-900 flex items-center bg-white border border-slate-200 px-3.5 py-2 rounded-xl shadow-xs transition-all">
        <i data-lucide="arrow-left" class="w-4 h-4 mr-1.5"></i> Kembali ke Kelola User
    </a>
</div>

<div class="bg-white rounded-3xl border border-slate-200/80 shadow-sm overflow-hidden max-w-4xl" x-data="userManager('<?= $init_role_kode ?>')">
    <form action="<?= BASE_URL ?>/index.php?page=users/process" method="POST" class="p-6 sm:p-8 space-y-6">
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    <label class="block text-xs font-semibold uppercase tracking-wider text-slate-600 mb-1.5">Role / Peran Pengguna <span class="text-rose-500">*</span></label>
                    <select name="role_id" @change="onRoleChange($event)" required class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none text-sm font-bold bg-white transition-all">
                ...
                <div>
                    <label class="block text-xs font-semibold uppercase tracking-wider text-slate-600 mb-1.5">NIP / Username <span class="text-rose-500">*</span></label>
                ...
                <div>
                    <label class="block text-xs font-semibold uppercase tracking-wider text-slate-600 mb-1.5">Password <span class="text-xs text-slate-400 font-normal lowercase">(kosongkan jika tidak diubah)</span></label>
