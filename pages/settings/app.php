...
<div class="mb-6 flex items-center justify-between">
    <div>
        <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">Pengaturan Tanda Tangan Laporan</h1>
        <p class="text-xs font-medium text-slate-500 mt-1">Atur nama, NIP, jabatan, dan visibilitas blok tanda tangan pada laporan Renja.</p>
    </div>
</div>
...
<div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden max-w-xl">
    <form action="<?= BASE_URL ?>/index.php?page=settings/process_app" method="POST" class="p-6 sm:p-8 space-y-5">
        ...
        <div class="flex items-center justify-between px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl">
            <div>
                <p class="text-sm font-bold text-slate-800">Tampilkan Tanda Tangan Pimpinan</p>
                <p class="text-xs text-slate-400 mt-0.5">Jika dinonaktifkan, kolom "Mengetahui" tidak akan muncul di laporan.</p>
            </div>
        ...
        <div id="ttd_fields" class="space-y-5 <?= !$tampilkan_pimpin ? 'opacity-40 pointer-events-none' : '' ?>">
            ...
            <div>
                ...
                <p class="text-xs text-slate-400 mt-1">Nama ini akan ditampilkan di bawah ruang tanda tangan kolom "Mengetahui".</p>
            </div>
            ...
            <div>
                ...
                <input type="text" name="penandatangan_nip" id="input_nip" value="<?= e($nip) ?>"
                    ...
                    class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none text-sm font-mono transition-all"
            ...
            <div>
                ...
                <p class="text-xs text-slate-400 mt-1">Jabatan ini akan tampil di atas ruang tanda tangan kolom "Mengetahui".</p>
            </div>

        </div>

        <!-- Preview -->
        <div class="bg-slate-50/80 rounded-2xl border border-slate-200/80 p-5">
            <p class="text-xs font-bold uppercase tracking-wider text-slate-500 mb-4">Preview Blok Tanda Tangan</p>
            <div class="flex justify-between text-center text-xs font-sans" id="preview_block">
                <div class="w-5/12" id="preview_pimpinan_col" style="<?= !$tampilkan_pimpin ? 'display:none' : '' ?>">
                    <p class="text-slate-700">Mengetahui,</p>
                    <p class="font-bold uppercase text-slate-900 mt-0.5" id="preview_jabatan"><?= e($jabatan ?: 'Kepala CDK Wilayah Nganjuk') ?></p>
                    <div class="h-12 border-b border-dashed border-slate-300 mt-4 mb-2"></div>
                    <p class="font-bold underline uppercase text-slate-900" id="preview_nama"><?= e($nama ?: '...') ?></p>
                    <p class="text-slate-500 font-mono mt-0.5">NIP. <span id="preview_nip"><?= e($nip ?: '-') ?></span></p>
                </div>
                <div id="preview_penyuluh_col" class="<?= $tampilkan_pimpin ? 'w-5/12' : 'w-full text-right' ?>">
                    <p class="text-slate-700">Nganjuk, [Tanggal Akhir Bulan]</p>
                    <p class="font-bold text-slate-900 mt-0.5">Penyuluh Kehutanan</p>
                    <div class="h-12 border-b border-dashed border-slate-300 mt-4 mb-2"></div>
                    <p class="font-bold underline uppercase text-slate-900">Nama Penyuluh</p>
                    <p class="text-slate-500 font-mono mt-0.5">NIP. xxxxxxxxxxxxxxxx</p>
                </div>
            </div>
            <p class="text-xs text-slate-400 mt-3 italic" id="preview_note" style="<?= $tampilkan_pimpin ? 'display:none' : '' ?>">
                * Hanya tanda tangan penyuluh yang akan ditampilkan.
            </p>
        </div>

        <div class="flex justify-end pt-2">
            <button type="submit" class="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold text-sm transition-all shadow-md shadow-indigo-500/20 active:scale-[0.98]">
