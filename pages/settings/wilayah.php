...
<div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between">
    <div>
        <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">Pengaturan Wilayah Kerja CDK</h1>
        <p class="text-xs font-medium text-slate-500 mt-1">Pilih Kabupaten / Kota di Jawa Timur yang aktif ditampilkan pada pilihan aplikasi.</p>
    </div>
    <div class="mt-4 sm:mt-0">
        <span class="inline-flex items-center text-xs font-semibold text-indigo-700 bg-indigo-50 px-3 py-1.5 rounded-full border border-indigo-100">
            <i data-lucide="map-pin" class="w-3.5 h-3.5 mr-1.5 text-indigo-600"></i>
...
<div class="bg-white rounded-3xl border border-slate-200/80 shadow-sm overflow-hidden" x-data="wilayahSetting()">
    <form action="<?= BASE_URL ?>/index.php?page=settings/process_wilayah" method="POST" class="p-6 sm:p-8 space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100">
            <div>
                <h3 class="text-base font-bold text-slate-900 tracking-tight">Daftar Kabupaten / Kota (Jawa Timur)</h3>
                <p class="text-xs text-slate-500 mt-0.5">Kabupaten yang dicentang akan menjadi pilihan aktif pada form kegiatan & KTH.</p>
            </div>
            
            <div class="flex items-center space-x-2">
                <button type="button" @click="selectOnlyNganjuk()" class="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-xl text-xs transition-all border border-slate-200">
                    Pilih Nganjuk Saja
                </button>
                <button type="button" @click="selectAll()" class="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-xl text-xs transition-all border border-slate-200">
                    Pilih Semua
                </button>
                <button type="button" @click="selectNone()" class="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-xl text-xs transition-all border border-slate-200">
                    Kosongkan
                </button>
            </div>
        </div>
        ...
        <div>
            <input type="text" x-model="searchQuery" placeholder="Cari nama Kabupaten / Kota..." 
                class="w-full max-w-md px-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition-all">
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 max-h-[500px] overflow-y-auto pr-1">
            ...
            <label x-show="matchesSearch('<?= e(addslashes($kab['nama'])) ?>')" 
                ...
                :class="isKabChecked(<?= $kab['id'] ?>) ? 'bg-indigo-50/70 border-indigo-200 shadow-2xs' : 'bg-slate-50/50 border-slate-200/80 hover:bg-slate-50'">
                ...
                <div class="flex items-center min-w-0 mr-2">
                    ...
                    <span class="ml-2.5 text-xs font-semibold text-slate-800 truncate" title="<?= e($kab['nama']) ?>"><?= e($kab['nama']) ?></span>
                </div>

                <span class="text-[10px] font-mono text-slate-400 font-medium"><?= e($kab['kode']) ?></span>
            </label>
            <?php endforeach; ?>
        </div>
        ...
        <div class="flex justify-between items-center pt-6 border-t border-slate-100">
            <span class="text-xs font-medium text-slate-500">
                <span class="font-bold text-slate-900" x-text="checkedKabIds.length"></span> Kabupaten dipilih
            </span>
